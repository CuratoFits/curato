...
class SQLAlchemyInteractionRepository(InteractionRepository):
    ...
    def get_all(self,skip: int = 0,limit: int = 20) -> list[InteractionEvent]:
        ...
        return self.db.scalars(select(InteractionEvent).offset(skip).limit(limit)).all()

    def get_by_id(self,event_id: int) -> InteractionEvent | None:

        return self.db.get(InteractionEvent,event_id)

    def get_by_user_id(self,user_id: int,skip: int = 0,limit: int = 20) -> list[InteractionEvent]:

        return self.db.scalars(select(InteractionEvent).where(
    ...
    def create(self,interaction: InteractionEventCreate) -> InteractionEvent:
        ...
        event = InteractionEvent(**interaction.model_dump())
    ...
    def update(self,event_id: int,interaction: InteractionEventUpdate) -> InteractionEvent | None:
