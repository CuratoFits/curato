...
def get_products(skip: int = Query(default=0, ge=0),limit: int = Query(default=20,ge=1,le=100),service: ProductService = Depends(
get_product_service)):
    return service.get_products(skip=skip,limit=limit)

@router.get("/category/{category}",response_model=list[ProductResponse])
def get_products_by_category(category: str,skip: int = 0,limit: int = 20,service: ProductService = Depends(get_product_service)):
